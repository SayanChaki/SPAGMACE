...
from torch.distributions import Normal, Uniform, Poisson, VonMises
...
class gmair(nn.Module):
    ...
    def _build_networks(self):
        ...
        self.feature_space_dim = [64,32,32]'''
    ...
    def _build_priors(self):
        ...
        for z_name, (mean, std) in cfg.priors.items():
         if z_name == 'theta':
            # Use von Mises distribution for theta
            # Assuming std here is actually the concentration parameter (kappa)
            # If std is variance, you might need to convert it to concentration kappa
            dist = VonMises(loc=mean, concentration=std)
         else:
            # Use normal distribution for other parameters
    ...
    def forward(self, x, global_step = -1, mode = 'train'):
        ...
        print(feat.shape)
        pres_logits = self.heat_map_head(feat)
        print("HELLO")
        #print(pres_logits)
        
        # where_latent : [batch, H, W, 8]
        where_latent = self.where_head(feat).permute(0,2,3,1).contiguous()
        #where_latent = self.where_head(feat)
        print("WHERE")
        print("WHERE")
        print("WHERE")
        print(where_latent.shape)
        ...
        print("Z8WHERE SHAPE IS BEING PRINTED")
        print(z_where.shape)
        ...
        _, H, W = self.feature_space_dim
        z_where_special = z_where.view(-1, 4)  # Adjust to match the expected shape [batch_size * H * W, 5]
        print("AFTER Resizing z_where")
        print(z_where_special.shape)
        input_glimpses = stn(x.repeat_interleave(H*W, dim = 0), z_where_special, cfg.object_shape, self.device)
    ...
    def _build_where(self, where_latent):
     ...
     # BN-VAE
     _, H, W = self.feature_space_dim
     mean = mean.contiguous().view(-1, H * W * 4)  # Adjusted to 5 for (y, x, h, w, theta)
     mean = self.box_mu_bn(mean)
     mean = mean.view(-1, H, W, 4)

     # Split the means and stds
     #offset_y_mean, offset_x_mean, scale_y_mean, scale_x_mean, theta_mean = torch.chunk(mean, 5, dim=-1)
     #offset_y_std, offset_x_std, scale_y_std, scale_x_std, theta_std = torch.chunk(std, 5, dim=-1)
     offset_y_mean, offset_x_mean, scale_y_mean, scale_x_mean = torch.chunk(mean, 4, dim=-1)
     offset_y_std, offset_x_std, scale_y_std, scale_x_std = torch.chunk(std,4 , dim=-1)
     ...
     scale_y_logits = self._sample_z(scale_y_mean, scale_y_std, 'scale_y')
     scale_x_logits = self._sample_z(scale_x_mean, scale_x_std, 'scale_x')
     #theta_logits = self._sample_z(theta_mean, theta_std, 'theta')
     '''print(offset_y_logits[0])
     print(offset_x_logits[0])     
     print(scale_y_logits[0])
     print(scale_x_logits[0])
     print(theta_logits[0])'''
     #print(theta_logits)
     ...
     # --- angle transform ---
     '''angle = torch.tanh(theta_logits) * torch.pi  # Example: Scale to range [-pi, pi]
     print("ANGLE.shape")
     print(angle.shape)'''
     #angle = torch.full(([16, 16, 16, 1]), 0.0).to('cuda')
     ...
     # Box centre mapped with respect to the full image
     _, H, W = self.feature_space_dim
     h = torch.linspace(0.5, H - 0.5, steps=H).to(self.device)
     w = torch.linspace(0.5, W - 0.5, steps=W).to(self.device)
     ...
     # Make sure angle has the same number of dimensions as xt, yt, xs, ys
     angle = angle.expand(-1, -1, -1, 1)  # Expand angle to [batch_size, H, W, 1]
 
     # Concatenate bounding box parameters and angle
     z_where = torch.cat([xt, yt, xs, ys, angle], dim=-1)
     return z_where
    ...
    def _sample_z(self, mean, var, name):
        ...
        if name == 'theta':
         kappa = var
         self.dist[name] = VonMises(loc=mean, concentration=kappa)
         return self.dist[name].sample()
        else:
    ...
    def _render(self, z_what, z_where, z_depth, obj_prob, x):
        ...
        print("BEFORE GOING IN UPDATE FUNCTION")
        print(z_where.shape)  
    ...
    def _update_z_where(self, objects, z_where):
     ...
     # Update x, y, h, w while keeping theta unchanged
     upd_z_where[:, 0] = z_where[:, 2] / px * (objects_real_y0 + objects_real_y1 - px) / 2 + z_where[:, 0]  # Update y (center)
     upd_z_where[:, 1] = z_where[:, 3] / px * (objects_real_x0 + objects_real_x1 - px) / 2 + z_where[:, 1]  # Update x (center)
     upd_z_where[:, 2] = z_where[:, 2] / px * (objects_real_y1 - objects_real_y0)  # Update height (h)
     upd_z_where[:, 3] = z_where[:, 3] / px * (objects_real_x1 - objects_real_x0)  # Update width (w)
    
     # Ensure theta remains unchanged
     upd_z_where[:, 4] = z_where[:, 4]  # theta remains the same
    
     # Reshape back to the original dimensions
     _, H, W = self.feature_space_dim
     upd_z_where = upd_z_where.view(-1, H, W, 4)  # Now the last dimension is 5 (x, y, h, w, theta)
    ...
    def _compute_KL(self, cls_logits, z_cls, prob, obj_prob):
        ...
        KL = {}
        import torch
        from torch.special import i0  # Bessel function

        def kl_divergence_von_mises_2d(p, q):
            mu_p, kappa_p = p.loc, p.concentration
            mu_q, kappa_q = q.loc, q.concentration
    
            # Compute the cosine of the difference in means
            cos_diff = torch.cos(mu_p - mu_q)
            '''print("KAPPPA VALUE")
            print(kappa_q)
            print(kappa_p)'''
    
            # Compute the log of the ratio of i0 values in a more numerically stable manner
            log_i0_ratio = torch.log(i0(kappa_q) + 1e-10) - torch.log(i0(kappa_p) + 1e-10)
    
            # KL divergence formula for von Mises distributions
            kl_div = log_i0_ratio + (kappa_p - kappa_q * cos_diff)
    
            # Ensure the KL divergence is non-negative
        ...
        for dist_name in self.dist.keys():
            ...
            if dist_name == 'theta':
                 kl_div = kl_divergence_von_mises_2d(dist, prior)
            else:
