...
class gmair(nn.Module):
    ...
    def _build_networks(self):
        ...
        self.feature_space_dim = [256,8,8]
        #self.backbone = get_resnet_dcn(18)
        #self.feature_space_dim = self.backbone.output_shape()
        print("FEATURE DIMENSIONS ARE AS FOL")
        print(self.feature_space_dim)
    ...
    def forward(self, x, global_step = -1, mode = 'train'):
        ...
        # feat : [batch, C, H, W]
        feat_geo = self.backbone(x)[2]
        feat=example_reresnet(feat_geo)
    ...
    def _compute_KL(self, cls_logits, z_cls, prob, obj_prob):
        ...
        var_prior = F.softplus(self.what_var_prior(z_cls))+epsilon
        
        if self.global_step <= cfg.freezing_cat_iters:
            mu_prior = mu_prior.detach()
            var_prior = var_prior.detach()
            
        self.kl_priors['what'] = Normal(loc=mu_prior, scale=torch.sqrt(var_prior + epsilon))
        ...
        pres_kl = obj_prob * (torch.log(obj_prob + epsilon) - self.log_prior_obj) \
                  + (1 - obj_prob) * (torch.log(1 - obj_prob + epsilon) - np.log(1 - self.prior_obj + epsilon))
                  
        log_cls = F.log_softmax(cls_logits, dim = -1)
        cls_kl = torch.sum(prob * log_cls, dim = -1, keepdim = True) + np.log(cfg.num_classes+epsilon)
